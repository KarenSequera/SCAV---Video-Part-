...
En la carpeta practice1 se encuentra la API que proporciona varias funcionalidades de procesamiento de imágenes como conversión de color, cambios de resolución, compresión mediante técnicas como la Transformada Discreta del Coseno (DCT) y la Transformada Discreta de Wavelet (DWT), entre otros. 

Para la API hemos usado el framework de Flask, ya que teníamos experiencia de otros trabajos. Hemos adaptado el código del anterior seminario (Seminar1) para que el input pueda introducirse mediante peticiones POST. Hay un endpoint POST para cada una de las funcionalidades y un endpoint GET para ejecutar el Unit Test. 

La API está dentro de un contenedor con base Linux Ubuntu, con todas las dependencias necesarias para ejecutarse y tiene abierto el puerto 5000 para poder interactuar con ella. Esta, se conecta con otro contenedor que contiene ffmpeg (tambien con base Linux Ubuntu) y lo usa para ejecutar los comandos de cambio de resolución y transformación a blanco y negro. Para poder instanciar los contenedores correctamente, permitir que se comuniquen entre sí y darles acceso al directorio compartido dónde se almacenan los inputs y los outputs (/shared/), se ha creado un archivo de configuración llamado docker-compose.yml. Este archivo contiene toda la información necesaria para configurar, montar y gestionar los contenedores: se especifica la localización de las diferentes dockerfiles, las interfaces de redes (para que se puedan comunicar) y volúmenes (dónde se monta la carpeta local de /shared/ a los contenedores). Para que el contenedor de ffmpeg esté siempre ejecutándose, se le indica que ejecute un proceso infinito que no ejecuta ninguna acción, de lo contrario la API no podría acceder a esa instancia. Para instanciar los contenedores usando el archivo de configuración,  usaremos el comando docker-compose.
...
Los contenidos de esta práctica se encuentran dentro de la carpeta practice1:
...
Ejemplo de petición POST (en PowerShell), se usarán jsons para la comunicación de inputs:

     $ Invoke-WebRequest -Uri http://localhost:5000/<end point de la función que se usa> -Method Post - Headers @{“Content-Type” = “application/json”} -Body’{{"Input1": "Datos", …}}’ - ContentType “application/json” 
