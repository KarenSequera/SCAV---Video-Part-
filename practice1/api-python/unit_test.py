...
class TestColorTranslator(unittest.TestCase):
    ...
    def test_rgb_to_yuv(self):
        translator = app.Color_Translator()
...
class TestSerpentine(unittest.TestCase):
    ...
    def test_serpentine(self):
        ...
        result = app.funcion_serpentine(matriz)
...
class TestRunLength(unittest.TestCase):
    ...
    def test_run_length(self):
        ...
        result = app.funcion_run_lenght(lista)
        self.assertEqual(result, expected_result)
...
def ejecutar_tests_resolution_changer():
    ...
    archivos = ['/shared/unit_tests/bosque.jpg', '/shared/unit_tests/gato.jpg', '/shared/unit_tests/perro.jpg']
    ...
    for archivo in archivos:
        ...
        for ancho, alto in resoluciones:
            ...
            output_name = f'/shared/unit_tests/resize/output_{nombre_base}_{ancho}x{alto}.jpg'

            # Llamada a la función que cambia la resolución
            app.funcion_resolution_changer(archivo, output_name, ancho, alto)
...
def ejecutar_tests_bw():
    ...
    for archivo in archivos:
        ...
        output_name = f'/shared/unit_tests/bw/output_{nombre_base}.jpg'

        # Llamada a la función que convierte la imagen a blanco y negro
        app.funcion_bw_converter(archivo, output_name)
...
def ejecutar_tests_dct_encoder():
    ...
    for archivo in archivos:
        ...
        imagen_encoded = app.DCT_Encoder_Class.metodo_encode(imagen)
        imagen_decoded = app.DCT_Encoder_Class.metodo_decode(imagen_encoded)
        ...
        plt.savefig(f'/shared/unit_tests/DCT/outputDCT_{nombre_base}.png', bbox_inches='tight')
        plt.close()
...
def ejecutar_tests_dwt_encoder():
    ...
    for archivo in archivos:
        ...
        imagen_encoded = app.DWT_Encoder_Class.metodo_encode(imagen)
        imagen_decoded = app.DWT_Encoder_Class.metodo_decode(imagen_encoded)
        ...
        #En lugar de plt.show(), guardamos el grafico
        plt.savefig(f'/shared/unit_tests/DWT/outputDWT{nombre_base}.png', bbox_inches='tight')
        plt.close()
        ...
        fig.tight_layout()
        plt.savefig(f'/shared/unit_tests/DWT/DWT_decomposición{nombre_base}.png', bbox_inches='tight')
        plt.close()
